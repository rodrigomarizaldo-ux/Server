...
router.post("/push", requireAuth, async (req, res) => {
    ...
    const { machines = [], operators = [], rentals = [], } = req.body;
    try {
        // ── Machines ──────────────────────────────────────────────────────────────
        if (machines.length > 0) {
            ...
            await db.delete(cloudMachinesTable).where(and(eq(cloudMachinesTable.userId, userId), notInArray(cloudMachinesTable.id, machineIds)));
        }
        else {
        ...
        if (operators.length > 0) {
            ...
            await db.delete(cloudOperatorsTable).where(and(eq(cloudOperatorsTable.userId, userId), notInArray(cloudOperatorsTable.id, operatorIds)));
        }
        else {
        ...
        if (rentals.length > 0) {
            ...
            await db.delete(cloudRentalsTable).where(and(eq(cloudRentalsTable.userId, userId), notInArray(cloudRentalsTable.id, rentalIds)));
        }
        else {
