...
// ──────────────────────────────────────────────────────────────
// Banco de dados em memória: armazenamento simples de máquinas
// ──────────────────────────────────────────────────────────────
interface Machine {
  id: string;
  model: string;
...
const db = new Map<string, Machine>();
...
// ──────────────────────────────────────────────────────
router.get("/", (_req, res) => {
  const machines = Array.from(db.values()).sort(
...
router.post("/", (req, res) => {
  ...
  const duplicate = Array.from(db.values()).find(
...
router.put("/:id", (req, res) => {
  ...
  if (updates.serialNumber && updates.serialNumber !== machine.serialNumber) {
    const duplicate = Array.from(db.values()).find(
      (m) => m.serialNumber === updates.serialNumber && m.id !== machine.id,
    );
    if (duplicate) {
      res.status(400).json({
        error: "DUPLICATE_SERIAL",
